...
def SimMatGam(inputfile1, inputfile2, index_wt, CovMat, chrinterest, stdsim = True, progress=True):
    ...
    for i in range(no_traits):
        ...
        if stdsim:
            covxfile = "{}/Similarity matrix btw ind_{}.npy".format(os.getcwd(), trait_names[i])# output file 
            mat = correlation_from_covariance(cov_bet_indxxxxxxxx)
            np.save(covxfile, mat) # writes std sim matrices for chromosomes of interest to file
        else:
            covxfile = "{}/Similarity matrix btw ind_{}.npy".format(os.getcwd(), trait_names[i])# output file 
            np.save(covxfile, corrtempxxxxxxxxxxx) # writes std sim matrices for chromosomes of interest to file
